      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            ...
            <p className="text-3xl font-bold text-foreground">{aiPercentage}%</p>
          </div>
        </div>
        {/* Bar */}
        <div className="relative h-3 rounded-full overflow-hidden gradient-bar">
          <div
            className="absolute top-0 h-full w-1 bg-foreground rounded-full shadow-lg transition-all duration-700"
            style={{ left: `${aiPercentage}%` }}
          />
        </div>
              ...
              {data.issues.map((issue, i) => (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-md ${severityBadge[issue.severity]}`}>
                      {issue.severity}
                    </span>
                  </div>
                </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {data.clear.map((item, i) => (
                <div key={i} className="flex items-center gap-2 rounded-lg bg-success/5 border border-success/20 px-3 py-2">
