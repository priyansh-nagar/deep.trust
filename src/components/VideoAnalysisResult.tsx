...
const getVideoTypeIcon = (type: string) => {
  if (type === "Live Action") return <Film className="w-4 h-4" />;
  if (type === "Animation") return <MonitorPlay className="w-4 h-4" />;
    ...
    >
      ...
      <motion.div variants={item} className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-foreground text-sm">Analyzed Video</h3>
        ...
        {data.video_type && (
          <div className="px-6 py-3 bg-secondary/30 flex items-center gap-2">
            {getVideoTypeIcon(data.video_type)}
            <span className="text-xs font-medium text-foreground">Content Type: {data.video_type}</span>
            {data.video_metadata?.estimated_duration && (
              <span className="text-xs text-muted-foreground ml-auto">Duration: {data.video_metadata.estimated_duration}</span>
            )}
          </div>
        )}
      </motion.div>
        ...
        <motion.div variants={item} className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-border bg-primary/5">
            <h3 className="font-semibold text-foreground flex items-center gap-2">
              ...
              DeepTrust Video Analysis Report
            </h3>
            <p className="text-xs text-muted-foreground mt-1">Comprehensive forensic breakdown of video authenticity</p>
          </div>
      ...
      {data.video_metadata && (
        <motion.div variants={item} className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {[
              { label: "Resolution / Quality", value: data.video_metadata.resolution_quality },
              { label: "Content Type", value: data.video_metadata.content_type },
              { label: "Likely Source", value: data.video_metadata.likely_source },
              { label: "Suspected Tool", value: data.video_metadata.suspected_tool },
            ].map((row, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, x: -10 }}
